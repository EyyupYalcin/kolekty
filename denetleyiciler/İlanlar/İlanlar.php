<?php include_once('Servisler/Hizmet_talebi.php'); ?>
...
<?php
    ...
    $baslik = $_GET['hizmet_adi'] . " İlanları";
    ...
    if(getAktifRol() == "Destek"){ // istek gönderen destek rolünde ise görünürlük önemsizdir
        if(isset($_GET['filtrele'])){
            ...
            $ilanlar = get_hizmet_talebi_By_uzmanliklar($aranan_uzmanliklar, false);
        }else{
            $ilanlar = get_hizmet_talebi_where(" hizmet_id = '" . $hizmet['id'] . "'");
        }
    }else{ // değilse sadece onaylayan_kullanici != 0 olanları görebilir.
        if(isset($_GET['filtrele'])){
            $aranan_uzmanliklar = explode('-ve-', $_GET['filtrele']);
            $ilanlar = get_hizmet_talebi_By_uzmanliklar($aranan_uzmanliklar, true);
        }else{
            $ilanlar = get_hizmet_talebi_where(" hizmet_id = '" . $hizmet['id'] . "' AND onaylayan_kullanici != 0");
        }
    }
